package co.edu.javeriana.dotnetgenerator.utils
...
class UtilGenerator {
	...
	def CharSequence valueTemplate(Expression e){
		...
		switch e{
			...
			ViewInstance: '''return view('«toSnakeCase(e.type.typeSpecification.name)»', «generateArray(e)») '''
	...
	def CharSequence generateTailedElement(VariableReference vr) {
		...
		var current = vr.tail
		while(current!=null){
			accumulate += "." + generateReferencedElement(current).toString.toFirstUpper
	...
	//Nuevooooooo
	def CharSequence generateTailedElement(Reference vr) {
		var accumulate= ''''''
		if(vr.referencedElement instanceof Attribute || vr.referencedElement instanceof Method){
			accumulate+=''''''			
		}
	...
	def CharSequence generateTailedElementOmitFirst(VariableReference vr) {
		var accumulate= ''''''
		if(vr.referencedElement instanceof Attribute || vr.referencedElement instanceof Method){
			accumulate+=''''''			
		}
	...
	def CharSequence generateTailedElementOmitFirst(Reference vr) {
		var accumulate= ''''''
		if(vr.referencedElement instanceof Attribute || vr.referencedElement instanceof Method){
			accumulate+=''''''			
		}
	...
	def CharSequence generateLastTailedElement(VariableReference vr) {
		var accumulate= ''''''		
		var current = vr.tail
		while(current!=null){
			accumulate = "" + generateReferencedElement(current).toString.toFirstUpper
	...
	def CharSequence generateReferencedElement(Reference reference) {
		switch reference.referencedElement{
			...
			Method:'''«reference.referencedElement.name.toFirstUpper»(«getParameters(reference as ParameterizedReference)»)'''
	...
	/**
	 * This method generates a C#-formatted array to pass to an auto-generated method <br>
	 * in the page template.
	 * @param instance the ViewInstance which has the auto-generated method
	 * @return the C#-formatted array
	 * */
	def CharSequence generateArray(ViewInstance instance){
		var accumulate= ""
		var string= ""
		for(i : 0..<instance.parameters.size){
			accumulate += valueTemplate(instance.parameters.get(i)) + ", " 
		}
	...
	def String namedUrlForActionCall(ActionCall ac){
		ac.action.name+"/"+generateParametersActionCall(ac);
	}
	...
	def CharSequence valueTemplateForEntities(Expression e){
		if(e instanceof VariableReference){
			if(e.referencedElement.type.typeSpecification instanceof Entity){
				...
				return '''«IF hasTail(e)», new {«generateLastTailedElement(e)» = «generateTailedElement(e)»}«ENDIF»'''		


			}else{
				return '''«IF hasTail(e)»FFF«generateTailedElement(e)»«ELSE»YYYY«e.referencedElement.name»«ENDIF»'''
			}
		}
	...
	def CharSequence getDataType(Type type)
	'''«IF type.typeSpecification.typeSpecificationString.equalsIgnoreCase("email")»string«ELSE»«IF type.isCollection»«IF getCollectionString(type as ParameterizedType).equals("Array")»«(type as ParameterizedType).typeParameters.get(0).typeSpecification.typeSpecificationString»[]«ELSE»«getCollectionString(type as ParameterizedType)»<«(type as ParameterizedType).typeParameters.get(0).typeSpecification.typeSpecificationString»>«ENDIF»«ELSE»«generateType(type.typeSpecification.name)»«ENDIF»«ENDIF»'''
