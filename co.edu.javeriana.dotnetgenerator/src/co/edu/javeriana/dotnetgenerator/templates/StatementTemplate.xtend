...
class StatementTemplate {
	...
	def dispatch CharSequence writeStatement(Show statement) '''
		$location.path('/«(statement.expression as Instance).type.typeSpecification.typeSpecificationString.toLowerCase»/' + «FOR param : (statement.expression as Instance).parameters»«IF !(param instanceof NullValue)»«writeExpression(param)»«ENDIF»«ENDFOR»);
	'''
	def dispatch CharSequence writeStatement(
		Return statement) '''
